...
module.exports = {
  ...
  buildModelPagination: function (request) {
    ...
    var override = {
      ...
      sortDirection: request.sort_field_direction.toUpperCase(),
      limit: request.count
    };
    ...
    if (!_.isEmpty(request.index)) {
      ...
      if (override.sortDirection === 'DESC') {
        where.id.lte = request.index;
      } else {
        where.id.gte = request.index;
      }
      query.where = where;
    }
  ...
  mergeQueryParams: function(request, query) {
    ...
    var exclusions = ['count', 'index', 'sort_direction', 'sort'];
